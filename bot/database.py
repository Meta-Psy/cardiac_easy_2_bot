...
import logging
from admin import perform_database_import, create_database_backup
...
class TestResult(Base):
    ...
    )  # минимальная/легкая/умеренная/тяжелая/крайне_тяжелая
    ...
    fagerstrom_level = Column(
        String(50), nullable=True
    )  # очень_слабая/слабая/средняя/сильная/очень_сильная
    ...
    audit_level = Column(
        String(50), nullable=True
    )  # низкий/опасное/вредное/зависимость
    ...
    overall_cv_risk_level = Column(
        String(50), nullable=True
    )  # НИЗКИЙ/УМЕРЕННЫЙ/ВЫСОКИЙ/ОЧЕНЬ_ВЫСОКИЙ
...
class ActivityLog(Base):
    ...
    )  # start, survey_started, survey_completed, test_started, test_completed
...
Index(
    "idx_tests_telegram_id_completed", TestResult.telegram_id, TestResult.completed_at
)
Index(
    "idx_activity_telegram_id_timestamp", ActivityLog.telegram_id, ActivityLog.timestamp
)
Index(
    "idx_broadcast_type_created", BroadcastLog.broadcast_type, BroadcastLog.created_at
)
    ...
    def _save():
        ...
        try:
            ...
            if verification:
                ...
                raise Exception(
                    f"Пользователь с telegram_id={user.telegram_id} не найден после сохранения!"
                )
...
async def save_user_data(
    ...
    """Перенаправление на безопасную функцию"""
...
async def save_survey_data(telegram_id: int, state_data: Dict[str, Any]):
    ...
    def _save():
        ...
        try:
            ...
            old_survey = (
                db.query(Survey).filter(Survey.telegram_id == telegram_id).first()
            )
            if old_survey:
            ...
            survey = Survey(
                ...
                # Препятствия и советы (вопросы 17-18)
                prevention_barriers=safe_json_dump(
                    state_data.get("prevention_barriers", [])
                ),
            ...
            return {
                ...
                "demographic_complete": bool(
                    survey.age and survey.gender and survey.location
                ),
                "health_assessment_complete": bool(
                    survey.health_rating and survey.heart_disease and survey.cv_risk
                ),
            }
...
async def save_test_results(telegram_id: int, test_data: Dict[str, Any]):
    ...
    def _save():
        ...
        try:
            ...
            old_results = (
                db.query(TestResult)
                .filter(TestResult.telegram_id == telegram_id)
                .first()
            )
            if old_results:
            ...
            test_result = TestResult(
                ...
                hads_anxiety_level=(
                    simple_risk_category(
                        "hads_anxiety", test_data.get("hads_anxiety_score", 0)
                    )
                    if test_data.get("hads_anxiety_score") is not None
                    else None
                ),
                hads_depression_level=(
                    simple_risk_category(
                        "hads_depression", test_data.get("hads_depression_score", 0)
                    )
                    if test_data.get("hads_depression_score") is not None
                    else None
                ),
                # Остальные тесты
                burns_score=test_data.get("burns_score"),
                burns_level=(
                    simple_risk_category("burns", test_data.get("burns_score", 0))
                    if test_data.get("burns_score") is not None
                    else None
                ),
                isi_score=test_data.get("isi_score"),
                isi_level=(
                    simple_risk_category("isi", test_data.get("isi_score", 0))
                    if test_data.get("isi_score") is not None
                    else None
                ),
                stop_bang_score=test_data.get("stop_bang_score"),
                stop_bang_risk=(
                    simple_risk_category(
                        "stop_bang", test_data.get("stop_bang_score", 0)
                    )
                    if test_data.get("stop_bang_score") is not None
                    else None
                ),
                ess_score=test_data.get("ess_score"),
                ess_level=(
                    simple_risk_category("ess", test_data.get("ess_score", 0))
                    if test_data.get("ess_score") is not None
                    else None
                ),
                ...
                fagerstrom_level=(
                    simple_risk_category(
                        "fagerstrom", test_data.get("fagerstrom_score", 0)
                    )
                    if test_data.get("fagerstrom_score") is not None
                    else None
                ),
                fagerstrom_skipped=test_data.get(
                    "fagerstrom_skipped", test_data.get("fagerstrom_score") is None
                ),
                audit_score=test_data.get("audit_score"),
                audit_level=(
                    simple_risk_category("audit", test_data.get("audit_score", 0))
                    if test_data.get("audit_score") is not None
                    else None
                ),
                audit_skipped=test_data.get(
                    "audit_skipped", test_data.get("audit_score") is None
                ),
                # Общий риск
                ...
                details=json.dumps(
                    {
                        ...
                        "tests_count": len(
                            [
                                ...
                                if v is not None and not k.endswith("_skipped")
                            ]
                        ),
                    },
                    ensure_ascii=False,
            ...
            if verification:
                logger.info(
                    f"✅ ТЕСТЫ СОХРАНЕНЫ: ID={verification.id}, риск={verification.overall_cv_risk_level}"
                )
                return {
                    ...
                    details=json.dumps(
                        {
                            ...
                            "user_journey": {
                                ...
                                "time_to_completion": (
                                    str(current_time - user.created_at)
                                    if user.created_at
                                    else None
                                ),
                            },
    ...
    try:
        ...
        with pd.ExcelWriter(filename, engine="openpyxl") as writer:
            ...
            user_columns = [
                ...
                if not (col.startswith("s.") or col.startswith("t."))
            ]
            ...
            # Результаты опросов
            survey_columns = ["telegram_id", "name"] + [
                col
                for col in full_data.columns
                if "age" in col or "gender" in col or "health" in col or "cv_" in col
            ]
...
def clean_old_data(days: int = 30) -> Dict[str, int]:
    ...
    try:
        ...
        old_broadcast_logs = (
            db.query(BroadcastLog).filter(BroadcastLog.created_at < cutoff_date).count()
        )

        db.query(BroadcastLog).filter(BroadcastLog.created_at < cutoff_date).delete()

        # Удаляем старую системную статистику (оставляем последние 90 дней)
        stats_cutoff = datetime.now() - timedelta(days=90)
        old_system_stats = (
            db.query(SystemStats).filter(SystemStats.date < stats_cutoff).count()
        )

        db.query(SystemStats).filter(SystemStats.date < stats_cutoff).delete()
...
async def log_user_activity(
    telegram_id: int, action: str, details: Dict[str, Any] = None, step: str = None
):
...
def get_comprehensive_user_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        new_users_today = (
            db.query(User).filter(db.func.date(User.created_at) == today).count()
        )
        ...
        active_today = (
            db.query(ActivityLog)
            .filter(db.func.date(ActivityLog.timestamp) == today)
            .distinct(ActivityLog.telegram_id)
            .count()
        )
...
def fix_incomplete_records():
    ...
    try:
        ...
        surveys_without_timestamps = (
            db.query(Survey)
            .filter(or_(Survey.created_at.is_(None), Survey.completed_at.is_(None)))
            .all()
        )
        ...
        tests_without_timestamps = (
            db.query(TestResult)
            .filter(
                or_(TestResult.created_at.is_(None), TestResult.completed_at.is_(None))
            )
            .all()
        )
...
def create_import_template() -> str:
    ...
    template_data = {
        ...
        "completed_diagnostic": [True, False],
        "registration_completed": [True, True],
        "survey_completed": [True, False],
        "tests_completed": [True, False],
        "age": [35, 28],
        "gender": ["Мужской", "Женский"],
        "location": ["Москва", "СПб"],
        "education": ["Высшее", "Среднее"],
        "health_rating": [7, 8],
        "hads_anxiety_score": [5, 3],
        "hads_depression_score": [4, 2],
        "overall_cv_risk_level": ["НИЗКИЙ", "НИЗКИЙ"],
    }

    # Создаем DataFrame
    df = pd.DataFrame(template_data)

    # Создаем Excel с инструкциями
    with pd.ExcelWriter(filename, engine="openpyxl") as writer:
        # Основной шаблон
        ...
        instructions = pd.DataFrame(
            {
                "Инструкции по импорту": [
                    '1. Заполните данные в листе "Шаблон импорта"',
                    "2. Обязательные поля: telegram_id, name, email, phone",
                    "3. Даты в формате: YYYY-MM-DD HH:MM:SS",
                    "4. Булевы значения: True/False или 1/0",
                    "5. Не изменяйте названия колонок",
                    "6. Можете добавить больше строк",
                    "7. Пустые ячейки будут проигнорированы",
                    "8. Дубликаты telegram_id будут заменены",
                    "",
                    "ВАЖНО: Создайте резервную копию перед импортом!",
                    "",
                    "Поддерживаемые колонки:",
                    "telegram_id - ID пользователя (обязательно)",
                    "name - Имя пользователя",
                    "email - Email адрес",
                    "phone - Номер телефона",
                    "completed_diagnostic - Завершена ли диагностика",
                    "registration_completed - Завершена ли регистрация",
                    "survey_completed - Завершен ли опрос",
                    "tests_completed - Завершены ли тесты",
                    "age - Возраст",
                    "gender - Пол",
                    "location - Местоположение",
                    "education - Образование",
                    "health_rating - Оценка здоровья (0-10)",
                    "... и все остальные поля из экспорта",
                ]
            }
        )
        instructions.to_excel(writer, sheet_name="Инструкции", index=False)
        ...
        references = pd.DataFrame(
            {
                ...
                "Уровень риска": ["НИЗКИЙ", "УМЕРЕННЫЙ", "ВЫСОКИЙ", "ОЧЕНЬ ВЫСОКИЙ"],
            }
        )
        references.to_excel(writer, sheet_name="Справочники", index=False)

    logger.info(f"Шаблон импорта создан: {filename}")
    return filename


# ============================================================================
# ФУНКЦИИ МИГРАЦИИ И ОБСЛУЖИВАНИЯ БД
# ============================================================================
...
def migrate_database_structure():
    ...
    try:
        ...
        for column, column_type in required_user_columns.items():
        ...
        for migration in migrations:
            try:
                ...
                logger.warning(f"Миграция не удалась: {migration}, ошибка: {e}")

        db.commit()
        logger.info(f"Миграция завершена. Выполнено операций: {len(migrations)}")

        return {"success": True, "migrations": len(migrations)}
...
def repair_database_integrity():
    ...
    try:
        ...
        if users_without_dates > 0:
            repairs.append(
                f"Исправлено created_at для {users_without_dates} пользователей"
            )

        # 2. Синхронизируем флаги completion с фактическими данными
        survey_sync = db.execute(
            text(
                """
            UPDATE users 
            SET survey_completed = TRUE 
            WHERE telegram_id IN (SELECT telegram_id FROM surveys)
        ...
        if survey_sync > 0:
            repairs.append(f"Синхронизировано {survey_sync} флагов survey_completed")

        test_sync = db.execute(
            text(
                """
            UPDATE users 
            SET tests_completed = TRUE 
            WHERE telegram_id IN (SELECT telegram_id FROM test_results)
            AND tests_completed = FALSE
        """
            )
        ).rowcount

        if test_sync > 0:
            repairs.append(f"Синхронизировано {test_sync} флагов tests_completed")

        # 3. Исправляем некорректные telegram_id (если есть)
        invalid_ids = db.execute(
            text(
                """
            SELECT COUNT(*) FROM users WHERE telegram_id < 100000 OR telegram_id > 9999999999
        """
            )
        ).fetchone()[0]

        if invalid_ids > 0:
            repairs.append(f"Найдено {invalid_ids} подозрительных telegram_id")
        ...
        if duplicates > 0:
            repairs.append(f"Удалено {duplicates} дубликатов пользователей")

        db.commit()
        logger.info(f"Исправление целостности завершено: {len(repairs)} операций")

        return {"success": True, "repairs": repairs, "repairs_count": len(repairs)}

    except Exception as e:
        db.rollback()
        logger.error(f"Ошибка исправления целостности: {e}")
        return {"success": False, "error": str(e)}
    finally:
        db.close()
...
# ============================================================================
# СПЕЦИАЛЬНЫЕ ФУНКЦИИ ДЛЯ РАССЫЛОК
# ============================================================================


async def get_filtered_users_advanced(filters: Dict[str, Any]) -> List[int]:
    """Получение пользователей с расширенными фильтрами"""
    ...
    def _get_users():
        ...
        try:
            ...
            # Базовые фильтры
            if filters.get("registration_completed", True):
                query = query.filter(User.registration_completed == True)

            if "completed_diagnostic" in filters:
                query = query.filter(
                    User.completed_diagnostic == filters["completed_diagnostic"]
                )

            if "survey_completed" in filters:
                query = query.filter(
                    User.survey_completed == filters["survey_completed"]
                )

            if "tests_completed" in filters:
                query = query.filter(User.tests_completed == filters["tests_completed"])

            # Фильтры по времени регистрации
            if "registered_after" in filters:
                query = query.filter(User.created_at >= filters["registered_after"])

            if "registered_before" in filters:
                query = query.filter(User.created_at <= filters["registered_before"])

            # Фильтры по активности
            if "active_after" in filters:
                query = query.filter(User.last_activity >= filters["active_after"])

            # Фильтры по результатам тестов (требует JOIN)
            if "risk_level" in filters:
                query = query.join(TestResult).filter(
                    TestResult.overall_cv_risk_level == filters["risk_level"]
                )

            # Фильтры по демографии (требует JOIN с Survey)
            if "age_min" in filters or "age_max" in filters or "gender" in filters:
                query = query.join(Survey)

                if "age_min" in filters:
                    query = query.filter(Survey.age >= filters["age_min"])

                if "age_max" in filters:
                    query = query.filter(Survey.age <= filters["age_max"])

                if "gender" in filters:
                    query = query.filter(Survey.gender == filters["gender"])
...
def create_incremental_backup() -> str:
    ...
    try:
        ...
        # Экспортируем в Excel для удобства
        excel_file = os.path.join(backup_dir, f"data_export_{timestamp}.xlsx")

        # Создаем краткий экспорт с основными данными
        db = get_db_sync()
        try:
            # Основные данные пользователей
            users_df = pd.read_sql(
                """
                SELECT telegram_id, name, email, phone, 
                       completed_diagnostic, survey_completed, tests_completed,
            ...
            recent_activity = pd.read_sql(
                """
                SELECT telegram_id, action, timestamp
                FROM activity_logs 
            ...
            with pd.ExcelWriter(excel_file, engine="openpyxl") as writer:
                ...
                recent_activity.to_excel(writer, sheet_name="Активность", index=False)
        ...
        logger.info(f"Инкрементальный бэкап создан: {archive_path}")
        return archive_path
...
def restore_from_backup(
    ...
    try:
        ...
        if backup_path.endswith(".db"):
            # Прямое восстановление из .db файла
            import shutil

            shutil.copy2(backup_path, "cardio_bot.db")
            restore_method = "database_file"

        elif backup_path.endswith((".xlsx", ".xls")):
            # Восстановление из Excel
            result = perform_database_import(backup_path)
            if not result["success"]:
                return result
            restore_method = "excel_import"
            ...
            # Ищем файл базы данных в архиве
            db_file = None
            ...
            if db_file:
                ...
                shutil.copy2(db_file, "cardio_bot.db")
                restore_method = "archive_extract"
            else:
                return {"success": False, "error": "Файл базы не найден в архиве"}
...
# ============================================================================
# ФУНКЦИИ АНАЛИЗА И ОПТИМИЗАЦИИ БД
# ============================================================================
...
def calculate_performance_score(table_sizes: Dict, total_records: int) -> str:
    ...
    try:
        ...
        # Штраф за несбалансированность таблиц
        activity_logs = table_sizes.get("activity_logs", {}).get("records", 0)
        users = table_sizes.get("users", {}).get("records", 1)  # Избегаем деления на 0

        if activity_logs > users * 100:  # Слишком много логов на пользователя
            score -= 20
...
def optimize_database() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            optimizations.append(f"Ошибка VACUUM: {e}")

        # 2. ANALYZE - обновление статистики запросов
        try:
            db.execute(text("ANALYZE"))
            optimizations.append("Обновлена статистика запросов (ANALYZE)")
        except Exception as e:
            optimizations.append(f"Ошибка ANALYZE: {e}")

        # 3. Переиндексация
        try:
            db.execute(text("REINDEX"))
            optimizations.append("Выполнена переиндексация (REINDEX)")
        except Exception as e:
            optimizations.append(f"Ошибка REINDEX: {e}")

        db.commit()

        # Получаем размер БД до и после оптимизации
        db_size_after = (
            os.path.getsize("cardio_bot.db") / 1024 / 1024
            if os.path.exists("cardio_bot.db")
            else 0
        )
...
# ============================================================================
# ФУНКЦИИ ДЛЯ ИМПОРТА ИЗ РАЗНЫХ ФОРМАТОВ
# ============================================================================


async def import_from_csv(file_path: str) -> Dict[str, Any]:
    ...
    def _import_csv():
        try:
            ...
            required_cols = ["telegram_id", "name", "email", "phone"]
            ...
            df = df.dropna(subset=["telegram_id"])
            df["telegram_id"] = df["telegram_id"].astype(int)

            # Импортируем через стандартную функцию
            # Конвертируем в формат Excel временно
            temp_excel = file_path.replace(".csv", "_temp.xlsx")
            df.to_excel(temp_excel, sheet_name="Все данные", index=False)

            result = perform_database_import(temp_excel)
    ...
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, _import_csv)
