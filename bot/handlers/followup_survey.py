...
async def start_followup_survey(callback: CallbackQuery, state: FSMContext):
    ...
    survey_completed = await asyncio.get_event_loop().run_in_executor(None, _check_survey_completed)

    # Если пользователь НЕ прошёл основной опрос - показываем приглашение
...
Берегите сердце, а за доказательной профилактикой — приходите к нам. Мы рядом 🫀"""
    ...
    try:
        ...
        # Приоритет - методичка менопаузы, fallback на ЗОЖ
        bonus_files = [
            ("materials/Методичка менопауза 1.pdf", "Руководство_для_женщин_40+.pdf", "📄 «Руководство для женщин 40+: как защитить сердце в перименопаузе и менопаузе»"),
