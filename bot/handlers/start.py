...
• Запись вебинара: https://novikova-diana.ru/kardiochekup_record
• Ваши персональные результаты диагностики
• Список анализов и рекомендации
• SCORE2 калькулятор для отслеживания риска

📋 <b>Доступные команды:</b>
/start - Главное меню и результаты диагностики
/status - Подробный статус и вся информация
...
📝 Отлично! Вы проходите диагностику. Это поможет получить персональные рекомендации на основе записи вебинара.

🔄 <b>Что делать:</b>
• Продолжайте честно отвечать на вопросы
• Каждый ответ влияет на точность оценки
• Процесс займет 10-15 минут
• После получите доступ ко всем материалам

📋 <b>Доступные команды:</b>
/status - Посмотреть текущий прогресс
/restart - Начать диагностику заново (если нужно)
/help - Эта справка

💡 <b>Советы:</b>
• Отвечайте честно - это поможет получить точные результаты
• Если не уверены в ответе, выбирайте наиболее близкий вариант
• После завершения получите персональную оценку рисков
...
🫀 Этот бот поможет вам получить максимум пользы от вебинара <b>"Умный кардиочекап"</b> с врачами-кардиологами.

🎥 <b>О вебинаре:</b>
• Состоялся 3 августа 2025 года
• Запись доступна: https://novikova-diana.ru/kardiochekup_record
• Длительность: полная версия с разбором случаев

🎯 <b>Что вы получите:</b>
• Персональную диагностику состояния сердечно-сосудистой системы
• Оценку ваших индивидуальных рисков и факторов
• Рекомендации основанные на результатах вебинара
• Бонусные материалы и список анализов
• Доступ к SCORE2 калькулятору риска

🚀 <b>Пошаговый план:</b>
1. Нажмите /start для начала
2. Введите контактные данные (имя, email, телефон)
3. Пройдите опрос о здоровье (18 вопросов, 5-7 минут)
4. Выполните психологические тесты (5 обязательных + 2 опциональных, 10-15 минут)
5. Получите персональные результаты и материалы
6. Изучите запись вебинара с учетом ваших результатов

📋 <b>Доступные команды:</b>
/start - Начать диагностику или главное меню
/status - Проверить статус и прогресс
/score - SCORE2 калькулятор сердечно-сосудистого риска  
/restart - Начать диагностику заново
/help - Эта подробная справка

💡 <b>Важно:</b> Диагностика займет всего 15-20 минут, но поможет получить персональные рекомендации на основе научных данных из вебинара!"""
...
📋 <b>ПРОГРЕСС ДИАГНОСТИКИ:</b>"""
            ...
            if user.survey_completed:
                ...
                if survey and survey.age:
                    text += f"\n   • Возраст: {survey.age} лет"
                    text += f"\n   • Пол: {survey.gender or 'не указан'}"
                    text += f"\n   • Место: {survey.location or 'не указано'}"
                    text += f"\n   • Образование: {survey.education or 'не указано'}"
                    if survey.completed_at:
                        text += f"\n   • Завершен: {survey.completed_at.strftime('%d.%m.%Y %H:%M')}"
            else:
                text += "\n❌ Опрос не пройден (0/18 вопросов)"
            
            # Тесты
            if user.tests_completed:
                text += "\n✅ Психологические тесты завершены"
                if tests:
                    text += f"\n   • Уровень тревоги: {getattr(tests, 'hads_anxiety_level', 'не определен')}"
                    text += f"\n   • Уровень депрессии: {getattr(tests, 'hads_depression_level', 'не определен')}"
                    text += f"\n   • Качество сна (ISI): {getattr(tests, 'isi_level', 'не определен')}"
                    text += f"\n   • Выгорание (Бернс): {getattr(tests, 'burns_level', 'не определен')}"
                    text += f"\n   • CV риск: {getattr(tests, 'overall_cv_risk_level', 'не определен')}"
                    if tests.completed_at:
                        text += f"\n   • Завершены: {tests.completed_at.strftime('%d.%m.%Y %H:%M')}"
            else:
                text += "\n❌ Психологические тесты не завершены"
...
🎥 <b>Полезные ссылки:</b>
• Запись вебинара: https://novikova-diana.ru/kardiochekup_record
• SCORE2 калькулятор: /score
• Помощь: /help

💡 <b>Команды:</b>
/start - Главное меню
/status - Этот статус  
/score - Калькулятор риска
/help - Справка"""
...
• Регистрация + опрос + тесты = 15-20 минут

💡 <b>Возможные причины для перезапуска:</b>
• Изменились ваши данные (возраст, состояние здоровья)
• Хотите получить более точные результаты
• Произошла ошибка в предыдущем прохождении

🎯 <b>После завершения вы получите:</b>
• Обновленные персональные результаты
• Актуальную оценку рисков
• Рекомендации на основе новых данных
...
async def start_welcome_flow(callback: CallbackQuery, state: FSMContext):
    ...
    text1 = """👋 Здравствуйте!
Добро пожаловать в бота по результатам вебинара «Умный Кардиочекап» с врачами-кардиологами Дианой Новиковой и Еленой Удачкиной.

Вы получили доступ к пошаговому алгоритму оценки риска развития сердечно-сосудистых заболеваний и их осложнений (в т.ч. инфаркт, инсульт, хроническая сердечная недостаточность), что позволит вовремя принять меры, сохранив здоровье и активность на годы вперёд без лишних затрат и избыточных обследований.

🎥 Вебинар состоялся 3 августа 2025 года  
📺 Запись доступна: https://novikova-diana.ru/kardiochekup_record

📍 Здесь, в боте, вы получите:
 — доступ к записи вебинара
 — список базовых анализов
 — чек-листы и бонусные материалы
 — персональную диагностику рисков
Мы уже всё приготовили 👌 Но сначала ― очень важный шаг, к которому мы просим отнестись серьёзно: без него вы не сможете получить максимум пользы от материалов.
